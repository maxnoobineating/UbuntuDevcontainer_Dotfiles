...
listVimProfile () {
	...
	grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} 'Total time:' $file | awk '{print $3}' | sort -nr | head -10 | sort -n | while read line
	do
		grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} -B 3 -A 1 "Total time:   $line" $file
		echo
	done
}
man () {
	...
	then
		shift
		command man "$@" 2> /dev/null > /dev/null
		if [[ $? -ne 0 ]]
		then
			if [[ -z "$1" ]]
			then
			...
			keyword="$@" 
			temp_file=$(mktemp /tmp/"$(echo $keyword | tr ' ' '_')"_help.XXXXXX)  || {
				echo "Failed to create temporary file"
				return 1
			}
			if bash -r -c "eval \"$keyword --help\"" | eval $MANPAGER > "$temp_file" 2>&1
			then
				echo "$temp_file"
			else
...
pwdc () {
	command -v xclip > /dev/null 2>&1 && pwd | xclip -selection clipboard || pwd > /mnt/c/clipboard.txt
}
pyenv_prompt_info () {
	return 1
}
...
export PATH=/root/.espressif/tools/python/v6.0.1/venv/bin:/usr/bin:/root/.espressif/tools/cmake/4.0.3/bin:/root/.espressif/tools/esp-clang/esp-20.1.1_20250829/esp-clang/bin:/root/.espressif/tools/esp-rom-elfs/20241011/:/root/.espressif/tools/esp32ulp-elf/2.38_20240113/esp32ulp-elf/bin:/root/.espressif/tools/esp32ulp-elf/2.38_20240113/esp32ulp-elf/esp32ulp-elf/bin:/root/.espressif/tools/ninja/1.12.1/:/root/.espressif/tools/openocd-esp32/v0.12.0-esp32-20260304/openocd-esp32/bin:/root/.espressif/tools/riscv32-esp-elf-gdb/16.3_20250913/riscv32-esp-elf-gdb/bin:/root/.espressif/tools/riscv32-esp-elf/esp-15.2.0_20251204/riscv32-esp-elf/bin:/root/.espressif/tools/riscv32-esp-elf/esp-15.2.0_20251204/riscv32-esp-elf/riscv32-esp-elf/bin:/root/.espressif/tools/xtensa-esp-elf-gdb/16.3_20250913/xtensa-esp-elf-gdb/bin:/root/.espressif/tools/xtensa-esp-elf/esp-15.2.0_20251204/xtensa-esp-elf/bin:/root/.espressif/tools/xtensa-esp-elf/esp-15.2.0_20251204/xtensa-esp-elf/xtensa-esp-elf/bin:/root/.espressif/tools/python/v6.0.1/venv/bin:/home/linuxbrew/.linuxbrew/bin:/home/linuxbrew/.linuxbrew/sbin:/root/.local/bin:/home/linuxbrew/.linuxbrew/bin:/home/linuxbrew/.linuxbrew/sbin:/root/.deno/bin:/root/.nvm/versions/node/v22.0.0/bin:/root/.local/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/root/.local/bin:/opt/nvim-linux-x86_64/bin:/root/go/bin:/usr/local/go/bin:/root/ctags:/root/.fzf/bin:/opt/nvim-linux-x86_64/bin:/root/go/bin:/usr/local/go/bin:/root/ctags:/root/.claude/plugins/cache/claude-plugins-official/clangd-lsp/1.0.0/bin
